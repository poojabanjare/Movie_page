...
async function fetchMovies(query) {
    try {
        ...
        console.log(`Fetching from URL: ${url}`); // Corrected the template literal
...
async function fetchMovieDetails(imdbID) {
    try {
        const response = await fetch(`${BASE_URL}?i=${imdbID}&apikey=${API_KEY}`);  // Corrected template literal
...
searchInput.addEventListener('input', () => {
    ...
    console.log(`Search Query: ${query}`); // Corrected the template literal
    if (query) {
        fetchMovies(query);
    } else {
